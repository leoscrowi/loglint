...
# Bonus Tasks:
## 1. **Configuration**: 
Rules configuration in `.golangci.yml`
```bash
settings:
  rules:
    - englishcheck
...
## 2. **Auto fixing**: 
`SuggestedFixes` for automatic error fixing: use `path-to-loglint-project.../bin/loglint run --fix`

## 3. **Custom patterns**: 
Added configuration field to
```bash
settings:
    keywords:
    - "ASD"
